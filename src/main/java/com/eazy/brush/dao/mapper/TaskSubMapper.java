...
public interface TaskSubMapper {
    ...
    @Select("Select "+FEILDS+" from task_sub where task_id=#{taskId},create_day=#{createDay} and state=2 and task_type=1 limit #{number}")
    List<TaskSub> getListByCreateDay(@Param("taskId")int taskId,@Param("createDay") int createDay,@Param("number")int number);
