...
public interface TaskSubMapper {
     ...
     * @param perTime
     * @param size
     * @see #getRandomCount(int,long)
     * @return
     */
    @Select("Select "+FEILDS+" from (Select (@rowNo :=@rowNo+1) AS rowno, task_sub.* from task_sub, (Select @rowNo := 0) b where create_day=#{createDay} and state=0 and per_time<=#{perTime}) tmp where rowNo>=#{randomCount} limit #{size}")
    List<TaskSub> getRandomList(@Param("createDay")int createDay,@Param("perTime") long perTime, @Param("randomCount")long randomCount, @Param("size") int size);

    @Select("Select count(*) from task_sub where create_day=#{createDay} and state=0 and per_time<=#{perTime}")
    int getRandomCount(@Param("createDay")int createDay,@Param("perTime")long perTime);
    ...
    @Select("select task_id," +"sum(temp_sum) as sum_time,"+
            "MAX(CASE WHEN task_type=0 then ocount else 0 end) as today_retain," +//留存数字
            "MAX(CASE WHEN task_type=1 then ocount else 0 end) as today_incr," +//新增数字
            "MAX(CASE WHEN task_type=2 then ocount else 0 end) as today_setup" +//唤醒数字
