...
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a class="btn btn-primary radius" href="javascript:;" onclick="add_station('添加站点','${ctx }/station/toAddStation/${line_id}','700','300')" ><i class="Hui-iconfont">&#xe600;</i> 添加站点</a></span> <span class="r">共有数据：<strong>88</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr>
				<th scope="col" colspan="6">${cityName } - ${lineName }</th>
			</tr>
			<tr class="text-c">
				<th width="40">ID</th>
		...
		<c:forEach items="${lineData }" var="d">
				<tr class="text-c">
					...
					<td>${d.create_time }</td>
					<td class="f-14">
						<a href="javascript:;" onclick="edit_station('编辑站点','${ctx }/station/toUpdateStation/${d.id}/${line_id}','700','300')" >编辑</a>
						<a href="${ctx }/station/deleteStation/${d.id}/${line_id}">删除</a>
					</td>
				</tr>
			</c:forEach>
...
/*添加城市*/
function add_station(title,url,w,h){
	layer_show(title,url,w,h);
}
function edit_station(title,url,w,h){
	layer_show(title,url,w,h);
}
$(function(){
